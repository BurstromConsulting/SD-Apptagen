...
$SD.on('sendToPropertyInspector', jsn => {
    ...
    if (pl.hasOwnProperty('error')) {
         ...
         *
         */
        if (pl.hasOwnProperty('response')) {
            var ele = document.getElementById('statusDropdown');
            for (var i = 0; i < pl.response.length; i++) {
                // BIND DATA TO <select> ELEMENT.
                if (pl.payload.length > 0 && pl.response[i]._id == pl.payload) {
                    ele.innerHTML = ele.innerHTML +
                        '<option value="' + pl.response[i]._id + '" selected="selected">' + pl.response[i].status + '</option>';
                }
                else {
                    ele.innerHTML = ele.innerHTML +
                        '<option value="' + pl.response[i]._id + '">' + pl.response[i].status + '</option>';
                }
            }
        }
    }
...
$SD.on('piDataChanged', (returnValue) => {
    ...
    console.log('%c%s', 'color: grey; background: blue}; font-size: 15px;', 'piDataChanged');
...
function handleSdpiItemChange(e, idx) {
    ...
    console.log("piData", returnValue);
